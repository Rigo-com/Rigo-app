...
const snapshot=Admin.snapshot();assert.equal(snapshot.state.running,true);assert.equal(snapshot.registry.modules.length,3);await Promise.race([Admin.shutdown(),new Promise((_,reject)=>setTimeout(()=>reject(new Error("ADMIN_SHUTDOWN_TIMEOUT")),5000))]);console.log("ADMIN_E2E_DONE");console.log("Admin end-to-end runtime test passed");process.exit(0);

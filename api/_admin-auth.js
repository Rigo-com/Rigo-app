...
function buildSession(email,persistent){
  return {email:normalizeEmail(email),role:"admin",persistent:Boolean(persistent),issuedAt:Date.now()};
}

function createToken(session){
  if(!SESSION_SECRET)throw new Error("ADMIN_SESSION_SECRET_NOT_CONFIGURED");
...
function verifyToken(token){
  try{
    ...
    if(session?.role!=="admin"||normalizeEmail(session?.email)!==ADMIN_EMAIL)return null;
    return session;
  }catch{return null;}
}

function cookieHeader(token,persistent){
  const parts=[`${COOKIE_NAME}=${encodeURIComponent(token)}`,"Path=/","HttpOnly","Secure","SameSite=Strict"];
  if(persistent)parts.push(`Max-Age=${PERSISTENT_MAX_AGE}`);
  return parts.join("; ");
}
...
export function authenticateAdmin(email,password){
  if(!adminAuthConfigured())return {ok:false,error:"ADMIN_AUTH_NOT_CONFIGURED"};
  if(!isAdminEmail(email))return {ok:false,error:"ADMIN_ACCESS_DENIED"};
  if(!safeEqual(String(password||""),ADMIN_PASSWORD))return {ok:false,error:"INVALID_ADMIN_CREDENTIALS"};
  return {ok:true,email:ADMIN_EMAIL};
}

export function issueAdminSession(response,{email=ADMIN_EMAIL,persistent=false}={}){
  const token=createToken(buildSession(email,persistent));
  response.setHeader("Set-Cookie",cookieHeader(token,persistent));
  return true;
}
...
export function requireAdminSession(request,response){
  ...
  if(!session){response.status(403).json({ok:false,error:"ADMIN_ACCESS_REQUIRED"});return null;}
